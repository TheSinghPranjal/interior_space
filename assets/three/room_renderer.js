...
(function () {
  ...
  class RoomRenderer {
    ...
    _updateWallVisibility() {
      ...
      const wallIds = Object.keys(this.wallMeshes);
      const hasPolygonFacing = wallIds.some((id) => {
        const mesh = this.wallMeshes[id];
        return mesh && mesh.userData.wallOutwardX != null;
      });
      ...
      if (hasPolygonFacing) {
        ...
        wallIds.forEach((id) => {
          const mesh = this.wallMeshes[id];
          if (!mesh || mesh.userData.wallOutwardX == null) return;
          const ox = mesh.userData.wallOutwardX;
          const oz = mesh.userData.wallOutwardZ;
          const wx = mesh.userData.wallMidX != null ? mesh.userData.wallMidX : mesh.position.x;
          const wz = mesh.userData.wallMidZ != null ? mesh.userData.wallMidZ : mesh.position.z;
          const toCamX = dx - wx;
          const toCamZ = dz - wz;
          const dist = Math.hypot(toCamX, toCamZ) || 1;
          const dot = (ox * toCamX + oz * toCamZ) / dist;
          if (dot > maxDot) {
            maxDot = dot;
            hideId = id;
          }
    ...
    _resolveRoomLayout(cfg) {
      ...
      let polygon = null;
      const rawPolygon = (room.floorPolygon && room.floorPolygon.length >= 3)
        ? room.floorPolygon
      ...
      if (rawPolygon &&
        ...
        polygon = this._preparePolygon(rawPolygon);
      }
      return {
    ...
    /** Match Flutter normalizedPolygonVertices: min-corner origin, then bbox center at world origin. */
    _preparePolygon(rawPoints) {
      const pts = rawPoints.map(p => ({
        x: Number(p.x) * FT,
        z: Number(p.y != null ? p.y : p.z) * FT,
      }));
      let minX = Infinity;
      let minZ = Infinity;
      let maxX = -Infinity;
    ...
    _polygonShape(polygon, flipZ) {
      const shape = new THREE.Shape();
      const shapeY = (p) => (flipZ ? -p.z : p.z);
      shape.moveTo(polygon[0].x, shapeY(polygon[0]));
      for (let i = 1; i < polygon.length; i++) {
        shape.lineTo(polygon[i].x, shapeY(polygon[i]));
      }
      return shape;
    }

    _polygonBoundsMeters(polygon) {
      let minX = Infinity;
      let maxX = -Infinity;
      let minZ = Infinity;
      let maxZ = -Infinity;
      ...
      return { w: maxX - minX, l: maxZ - minZ };
    }

    _buildPolygonFloor(polygon, floor) {
      // rotateX(-PI/2) mirrors Z; negate shape Y so footprint matches ceiling and walls.
      const shape = this._polygonShape(polygon, true);
      const geo = new THREE.ShapeGeometry(shape);
      geo.rotateX(-Math.PI / 2);
      const { w, l } = this._polygonBoundsMeters(polygon);
      ...
      const mat = this._makeMaterial(
        ...
        this._resolveTextureUrl(floor.textureDataUrl, floor.texturePath), null,
        w / (floor.tileWidth * FT), l / (floor.tileLength * FT)
      );
    ...
    _buildPolygonCeiling(polygon, h, ceiling) {
      const shape = this._polygonShape(polygon, false);
      const geo = new THREE.ShapeGeometry(shape);
      geo.rotateX(Math.PI / 2);
    ...
    _tagWallFacingData(obj, midX, midZ, outwardX, outwardZ) {
      obj.userData.wallMidX = midX;
      obj.userData.wallMidZ = midZ;
      obj.userData.wallOutwardX = outwardX;
      obj.userData.wallOutwardZ = outwardZ;
    }
    ...
    _buildRoom() {
      ...
      const layout = this._resolveRoomLayout(cfg);
      this.roomSize = { w: layout.w, l: layout.l, h: layout.h };
    ...
    _buildPolygonWallBarrierGroup(a, b, wallCfg, h, id) {
      ...
      const outwardX = dz / len;
      const outwardZ = -dx / len;
      const group = new THREE.Group();
      ...
      group.userData.wallId = id;
      this._tagWallFacingData(group, midX, midZ, outwardX, outwardZ);
      group.traverse((c) => {
        if (c.isMesh) {
          c.castShadow = true;
          c.receiveShadow = true;
    ...
    _buildPolygonWallSegment(a, b, wallCfg, h, mat, id) {
      ...
      mesh.userData.wallId = id;
      this._tagWallFacingData(mesh, midX, midZ, outwardX, outwardZ);
      mesh.receiveShadow = true;
      return mesh;
    }
