...
(function () {
  ...
  class RoomRenderer {
    ...
    _applyCameraMode(mode) {
      ...
      switch (mode) {
        case 'top':
          this.camera.position.set(cx, h * 2.5, cz);
          this.camera.lookAt(cx, 0, cz);
          break;
        case 'front':
          this.camera.position.set(cx, h * 0.5, l * 1.5);
          this.camera.lookAt(cx, h * 0.4, cz);
          break;
        case 'side':
          this.camera.position.set(w * 1.5, h * 0.5, cz);
          this.camera.lookAt(cx, h * 0.4, cz);
          break;
        case 'isometric':
          this.camera.position.set(w * 0.8, h * 1.2, l * 0.8);
          this.camera.lookAt(cx, h * 0.3, cz);
          break;
        case 'walk':
        case 'firstPerson':
          this.camera.position.set(cx, h * 0.15, l * 0.3);
          this.camera.lookAt(cx, h * 0.15, cz);
          break;
        default:
          this.camera.position.set(w * 0.9, h * 0.7, l * 1.2);
    ...
    _buildWalls(w, l, h, cfg) {
      ...
      wallDefs.forEach(def => {
        ...
        if (openings.length === 0) {
          ...
          this._buildWallWithOpenings(def, mat, openings);
        }
      });
    }
    ...
    _buildCupboards(cupboards, w, l, h) {
      cupboards.forEach(c => {
        ...
        const mat = this._makeMaterial(c.color, 0.6, 0.05, c.textureDataUrl, 'wood', 1, 1);
        const mesh = new THREE.Mesh(geo, mat);
        mesh.castShadow = true;
        mesh.receiveShadow = true;

        const pos = this._wallMountedPosition(c.wall, c.positionFromEdge, cw, cd, w, l);
        mesh.position.set(pos.x, ch / 2, pos.z);
        if (c.wall === 'left') mesh.position.x += cd / 2;
        if (c.wall === 'right') mesh.position.x -= cd / 2;
        if (c.wall === 'front') mesh.position.z += cd / 2;
        if (c.wall === 'back') mesh.position.z -= cd / 2;
        this.roomGroup.add(mesh);
      });
    }
    ...
    _buildFurniture(furniture, w, l) {
      furniture.forEach(item => {
        ...
        const proc = item.type === 'bed' || item.type === 'wardrobe' ? 'wood' : null;
        const mat = this._makeMaterial(item.color, 0.7, 0.05, null, proc, 1, 1);
        const mesh = new THREE.Mesh(geo, mat);
        mesh.castShadow = true;
        mesh.receiveShadow = true;
        ...
        mesh.position.set(x, fh / 2, z);
        mesh.rotation.y = (item.rotation || 0) * Math.PI / 180;
        this.roomGroup.add(mesh);

        if (item.type === 'bed') {
          const pillowGeo = new THREE.BoxGeometry(fw * 0.35, fh * 0.15, fd * 0.25);
          const pillowMat = new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.9 });
          const pillow = new THREE.Mesh(pillowGeo, pillowMat);
          pillow.position.set(x, fh + fh * 0.05, z - fd * 0.25);
