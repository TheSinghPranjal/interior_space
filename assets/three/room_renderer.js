  ...
  class RoomRenderer {
    ...
    _buildCupboards(cupboards, w, l, h) {
      cupboards.forEach((c) => {
        ...
        if (c.wall === 'left') group.position.x += cd / 2;
        if (c.wall === 'right') group.position.x -= cd / 2;
        if (c.wall === 'front') group.position.z += cd / 2;
        if (c.wall === 'back') group.position.z -= cd / 2;
        if (c.wall === 'left' || c.wall === 'right') group.rotation.y = Math.PI / 2;
        this.roomGroup.add(group);
      });
    }
      ...
      furniture.forEach((item) => {
        ...
        switch (item.type) {
          ...
          case 'cupboard':
            ...
            break;
          case 'sofa':
            group = this._buildSofaGroup(item);
            break;
          case 'table':
    ...
    _buildSofaGroup(item) {
      const fw = item.width * FT;
      const fh = item.height * FT;
      const fd = item.depth * FT;
      const group = new THREE.Group();
      const mat = this._makeMaterial(item.color, 0.85, 0.02, null, 'fabric', 1, 1);
