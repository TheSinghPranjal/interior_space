...
(function (global) {
  ...
  const FT = 0.3048;
  const EYE_HEIGHT = 5 * FT;
  const WALK_SPEED = 2.5 * FT;
  const RUN_SPEED = 5 * FT;
  ...
  class WalkthroughController {
    ...
    _applyWalkCamera(x, z, yaw, pitch, fov) {
      ...
      cam.position.set(x, EYE_HEIGHT, z);
    ...
    enter(previousMode) {
      ...
      const to = {
        position: new THREE.Vector3(spawn.x, EYE_HEIGHT, spawn.z),
    ...
    update(delta) {
      ...
      const speed = this.runActive ? RUN_SPEED : WALK_SPEED;
      let targetForward = 0;
      let targetRight = 0;
      ...
      const blend = 1 - Math.exp(-delta / ACCEL_TIME);
      ...
      cam.position.x += dx * delta;
      cam.position.z += dz * delta;
