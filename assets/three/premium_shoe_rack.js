...
(function (global) {
  ...
  /* ── material helper (emissive so it shows in dark scenes) ── */
  ...
  /* ══ colour palette ══ */
  // Dark walnut wood
  const WALNUT    = 0x3d1f0d;
  const WALNUT_E  = 0x1a0a04;
  const WALNUT_MID= 0x4a2510;
  const WALNUT_ME = 0x200e05;
  // Lighter walnut for door panels
  const WALNUT_LT = 0x5c2e12;
  const WALNUT_LE = 0x2a1208;
  // Dark frame / shadow areas
  const FRAME     = 0x2a1208;
  const FRAME_E   = 0x0d0603;
  // Brushed gold handles
  const GOLD      = 0xc8a96e;
  const GOLD_E    = 0x7a6030;
  // Marble top (dark emperor marble)
  const MARBLE    = 0x3a3530;
  const MARBLE_E  = 0x1a1510;
  const MARBLE_VN = 0x4a4540; // vein highlight
  ...
  /* ── brushed-gold cup handle ── */
  function addCupHandle(group, x, y, z, width, gm) {
    const r  = 0.008 * FT;
    const hw = width * 0.5;
    const bow= 0.018 * FT;   // how far the arc bows out

    // horizontal bar
    const bar = new THREE.Mesh(
      new THREE.CylinderGeometry(r, r, width, 14), gm);
    ...
    [-1, 1].forEach(s => {
      const post = new THREE.Mesh(
        new THREE.CylinderGeometry(r*0.7, r*0.7, bow, 10), gm);
  ...
  /* ── inset panel (recessed rectangle on door/drawer face) ── */
  function addInsetPanel(group, x, y, z, w, h, depth, framM, panM) {
    const border = 0.018 * FT;
    const pw = w - border*2;
    const ph = h - border*2;

    // outer frame face (thin box, same depth as door)
    const frame = rBox(w, h, depth, 0.004*FT, framM);
    frame.position.set(x, y, z);
    group.add(frame);

    // inset panel (slightly recessed, lighter colour)
  ...
  function buildPremiumShoeRack(renderer, item, _textureUrl) {
    ...
    const topSlabH= fh * 0.055;   // marble top slab
    const cabinetH= bodyH - topSlabH;

    // zones (top = drawers, bottom = doors)
    const drawerZoneH = cabinetH * 0.22;
    const doorZoneH   = cabinetH * 0.78;
    const nCols       = 3;
    const colW        = fw / nCols;
    const doorD       = 0.030 * FT;   // door protrusion from cabinet face
    ...
    [[-1,-1],[-1,1],[1,-1],[1,1]].forEach(([sx,sz]) => {
      ...
      group.add(post);
      // decorative notch at top of foot
    ...
    // horizontal divider between drawers and doors
    ...
    for (let col = 0; col < nCols; col++) {
      ...
      // drawer face with inset panel
      addInsetPanel(
        group,
        cx,
        drawerBaseY + drawerH*0.5,
        fd/2 + drawerD*0.5,
        dw, drawerH, drawerD,
        WM, WLM
      );

      // cup handle centered on drawer
      addCupHandle(group,
        cx,
        drawerBaseY + drawerH*0.5,
        fd/2 + drawerD + 0.010*FT,
        colW * 0.38,
        GM
      );

      // drawer gap shadow (top)
    ...
    for (let col = 0; col < nCols; col++) {
      ...
      // door face — two stacked inset panels for a classic look
      const topPanelH  = doorH * 0.30;
      const botPanelH  = doorH * 0.62;
      const panelGap   = doorH * 0.05;
      ...
      addInsetPanel(
        group,
        cx,
        doorBaseY + doorH - panelBorderV - topPanelH*0.5,
        fd/2 + doorD*0.55,
        dw * 0.82, topPanelH,
        doorD * 0.45,
        FM, WLM
      ...
      // bottom inset panel (taller)
      addInsetPanel(
        group,
        cx,
        doorBaseY + panelBorderV + botPanelH*0.5,
        fd/2 + doorD*0.55,
        dw * 0.82, botPanelH,
        doorD * 0.45,
        FM, WLM
      ...
      // cup handle (below top panel)
      addCupHandle(group,
        cx,
        doorBaseY + doorH - panelBorderV - topPanelH - 0.028*FT,
        fd/2 + doorD + 0.010*FT,
        colW * 0.38,
        GM
      );
    ...
    /* ════════ MARBLE / GRANITE TOP SLAB ════════ */
    const slabY  = footH + cabinetH;
    const slabW  = fw + 0.030*FT;    // overhangs slightly
    const slabD  = fd + 0.020*FT;

    const slab = rBox(slabW, topSlabH, slabD, 0.006*FT, MM);
    slab.position.set(0, slabY + topSlabH*0.5, 0);
    group.add(slab);

    // marble vein lines (procedural)
    const veinMat = mkMat(MARBLE_VN, 0x606050, 0.18, 0.04, 0.12);
    const veins = [
      { x:-slabW*0.28, angle:0.18, len:slabW*0.55 },
      { x: slabW*0.10, angle:-0.12, len:slabW*0.40 },
      { x: slabW*0.32, angle:0.25, len:slabW*0.30 },
      { x:-slabW*0.05, angle:0.05, len:slabW*0.65 },
    ];
    veins.forEach(v => {
      const vein = new THREE.Mesh(
        new THREE.BoxGeometry(v.len, topSlabH*1.02, 0.004*FT),
        veinMat
      );
      vein.rotation.y = v.angle;
      vein.position.set(v.x, slabY + topSlabH*0.5, 0);
      group.add(vein);
    });

    // slab edge highlight (lighter edge banding)
    ...
    const sheenTop = new THREE.Mesh(
      new THREE.PlaneGeometry(slabW*0.88, slabD*0.80),
      new THREE.MeshBasicMaterial({color:0xffffff, transparent:true, opacity:0.06, depthWrite:false})
    );
    sheenTop.rotation.x = -Math.PI/2;
    sheenTop.position.set(-slabW*0.08, slabY + topSlabH + 0.001*FT, -slabD*0.05);
    group.add(sheenTop);

    /* ════════ FACE TRIM / MOULDING ════════ */
    // top front moulding strip (under slab)
    ...
    // vertical wood grain highlight strip
    const grainHL = new THREE.Mesh(
      new THREE.PlaneGeometry(fw*0.04, cabinetH*0.80),
      new THREE.MeshBasicMaterial({color:0xffffff, transparent:true, opacity:0.04, depthWrite:false})
    );
    grainHL.position.set(-fw*0.15, footH + cabinetH*0.5, fd/2 + doorD + 0.002*FT);
    group.add(grainHL);
